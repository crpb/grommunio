...
# Copyright © 2023-2026 Christopher Bock <christopher@bocki.com>
...
# autoconfig         IN  CNAME            grommunio-host.domain.tld.
...
echo -e "\t${RET}"
# Check for CNAMEs
echo -e "Testing \033[0;32m$DOMAIN\033[0m CNAMEs"
for ENT in "${CNAME[@]}"; do
  NAME="${ENT}.${DOMAIN}"
  echo -e "Testing \033[0;32m$NAME\033[0m"
  RET="$($DIG +short +timeout=1 CNAME "$NAME" $NS || echo "Error: $NAME")"
  if [ ${#RET} -gt 1 ]; then
    printf '\t%s\n' "$RET"
  else
    echo -e "\t\033[0;35mNothing found\033[0m"
