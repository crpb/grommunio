...
group.add_argument('--years', metavar='N', type=int,
                   default=1, help='years in the past')
group.add_argument('--flagged', action=argparse.BooleanOptionalAction,
                   default=True)
group = parser.add_argument_group('options')
group.add_argument('--mark', action=argparse.BooleanOptionalAction,
                   default=False)
...
#If we don't want to take action we will see what we would have matched
    ...
    with MailBox(args.server).login(args.username, args.password) as mailbox:
        for f in mailbox.folder.list():
            mailbox.folder.set(f.name)
            for msg in mailbox.fetch(mark_seen=args.mark,
                                     criteria=crit,
                                     limit=args.limit):
                print(f.name, msg.date, msg.subject[0:30],
